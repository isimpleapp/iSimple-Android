...
public class MapFragment extends SupportMapFragment implements GoogleMap.OnInfoWindowClickListener {
    ...
    private void createOverlays(final boolean isRouteOverlay) {
        getMapAsync(new OnMapReadyCallback() {
            @Override
            public void onMapReady(GoogleMap map) {
                Location currentLocation = LocationTrackingManager.getInstante(getActivity()).getCurrentLocation(getActivity());
    ...
    private Marker createMarker(GoogleMap map, Shop shop) {
        return map.addMarker(new MarkerOptions().title(shop.getLocationName()).snippet(shop.getLocationAddress()).position(new LatLng((double) shop.getLatitude(), (double) shop.getLongitude())));
    }
    ...
    private class CreateRouteTask extends AsyncTask<LatLng, Void, PolylineOptions> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = ProgressDialog.show(getActivity(), getString(R.string.dialog_title),getString(R.string.load_route_dialog_progress_message), false, false);
        }
