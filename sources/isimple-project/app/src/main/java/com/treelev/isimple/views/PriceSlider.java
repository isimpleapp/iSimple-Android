...
public class PriceSlider extends ImageView {
	...
	public enum Region {
		Moscow("Москва"), MoscowRegion("Подмосковье"), StPetersburg(
				"Санкт-Петербург");
	...
	public void setRegion(Region region) {
		mRegion = region;
		if (mRegion == null) {
			Log.e(PriceSlider.class.getSimpleName(), "Region == null");
		} else {
			switch (mRegion) {
			case Moscow:
				initMoscow();
	...
	public void setValue(double value) {
		mAbsValue = value;
		if (mRegion == null) {
			Log.e(PriceSlider.class.getSimpleName(),
					"Set value mRegion == null");
		} else {
			switch (mRegion) {
			case Moscow:
				setValueMoscow();
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		initValues();
		if (mRegion == null) {
			Log.e(PriceSlider.class.getSimpleName(), "onDraw() mRegion == null");
		} else {
			switch (mRegion) {
			case Moscow:
	...
	private void drawFirstRange(Canvas canvas) {
		RectF rect = new RectF(mPadding,
				(float) (getAxisLine() - mHalfHeightLine), getWidth()
						- mPadding, (float) (getAxisLine() + mHalfHeightLine));
		...
		canvas.drawCircle(firstBorderCoord, (float) getAxisLine(), mCupRadius,
				mPaint);
		...
		canvas.drawCircle(secondBorderCoord, (float) getAxisLine(), mCupRadius,
				mPaint);
	...
	private void drawSecondRange(Canvas canvas) {
		...
		canvas.drawCircle(mPadding, (float) getAxisLine(), mInnerCupRadius,
				mPaint);
	...
	private double getAxisLine() {
		return getHeight() * 0.25f; // line y-axis 20% from the top to the
									// bottom
	}
	...
	private void initValues() {
		...
		mHalfHeightLine = 0.075f * getHeight(); // height line 15% for all
												// height view
	...
	private double normalizedToValue(double normalized) {
		return mAbsoluteMinValue + normalized
				* (mAbsMaxValue - mAbsoluteMinValue);
	}

	private float normalizedToScreen(double normalizedCoord) {
		return (float) (mPadding + normalizedCoord
				* (getWidth() - 2 * mPadding));
	}
