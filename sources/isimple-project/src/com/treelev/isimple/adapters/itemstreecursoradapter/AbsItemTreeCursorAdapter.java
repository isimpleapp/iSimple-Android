...
public abstract class AbsItemTreeCursorAdapter extends SimpleCursorTreeAdapter implements
    ...
    private void bindItemView(View view, Cursor cursor, ViewHolder viewHolder) {
        ...
        viewHolder.itemLocName
                .setText(organizeLocItemNameLabel(cursor.getString(itemLocNameIndex)));
        ...
        if (mGroup) {
            ...
            int drinkId = strDrinkId != null && strDrinkId.length() != 0 ? Integer
                    .valueOf(strDrinkId) : 1;
            if (drinkId > 1 && volumeLabel != null) {
                formatVolume = "%s товар%s";
                String end = "";
                if ((drinkId >= 10 && drinkId <= 20)
                        || strDrinkId.charAt(strDrinkId.length() - 1) == '0') {
                    end = "ов";
        ...
        String strDrinkCategory = DrinkCategory.getDrinkCategory(
                cursor.getInt(itemDrinkCategoryIndex))
                .getDescription();
        if (mYearEnable) {
